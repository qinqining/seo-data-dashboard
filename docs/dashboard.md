...
2. 按本文 **第 4 节** 创建 **8 张工作表**（业务上 5 个模块 + GSC 明细 2 表；「执行复盘」拆成日日志 + 周复盘两张表，字段差异大，不要硬塞一张表）。
...
A：**页面管理表**仍人工登记重点 URL；全站 Top 页面走 **「GSC Top 页面明细」**（§4.8），与页面表互补。
...
*文档版本：2026-06-04 · 新增 GSC Top 查询/页面明细两表（§4.7–4.8）*