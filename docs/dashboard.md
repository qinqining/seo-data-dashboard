...
| **all（enrich）**          | GSC Top 查询 Ahrefs 补数默认 **全部国家**；`.env` → `GSC_TOP_QUERIES_ENRICH_COUNTRIES` **留空**；填 `usa,gbr` 可限国 |
...
### 3.2 GSC Top 查询 / Top 页面 — Top N 与 Ahrefs enrich
...
| ---------------------- | ------------------- | -------------- | ----------------------- | ---------------------------------------- |
| **GSC Top 查询明细**（§4.7） | `query` × `country` | **1000** 行/站/日 | `GSC_TOP_QUERIES_LIMIT` | GSC 按点击 Top 1000 拉取后，**只入库点击 > 0**；非全量词库 |
| **GSC Top 页面明细**（§4.8） | `page` × `country`  | **1000** 行/站/日 | `GSC_TOP_PAGES_LIMIT`   | 同上，**只入库点击 > 0** 的 URL×国家                |
...
**Ahrefs enrich（仅 GSC Top 查询表）：** GSC 写入完成后自动执行 `enrich_gsc_top_queries`（可用 `--skip-gsc-top-queries-enrich` 跳过）。
...
| **仅锚点日**       | Backlinks变化、收录/异常数、异常预警、周环比流量、周平均排名                   | 锚点日汇总或 7 日 GSC 均值，只写在锚点那一行               |
...
**Q：伊朗词没有 Ahrefs搜索量/KD？**
A：预期行为。Ahrefs keywords-explorer 不支持伊朗；GSC Top 查询行仍会保留 GSC 指标，enrich 不补 Ahrefs 三列。

---

*文档版本：2026-06-10 · GSC Top enrich 国码映射已扩至 mingdao 国家选项；§3.2*