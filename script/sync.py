...
class SyncReport:
    ...
    keyword_grading_csv: Path | None = None
    ...
    def add_keyword_grading_report(
        self,
        lines: list[str],
        *,
        ...
        if csv_path is not None:
            self.keyword_grading_csv = csv_path
    ...
    def save(self) -> Path:
        ...
        if self.keyword_grading_lines:
            ...
            if self.keyword_grading_csv:
                lines.append(f"Grading CSV: {self.keyword_grading_csv}")
...
def sync_keywords(
    ...
    csv_path: Path | None = None
    if grade_details:
        csv_path = REPORT_DIR / f"keyword-grading-{site.key}-{data_date.isoformat()}.csv"
        fieldnames = list(grading_detail_csv_row(grade_details[0]).keys())
        with csv_path.open("w", encoding="utf-8-sig", newline="") as handle:
            writer = csv.DictWriter(handle, fieldnames=fieldnames)
            writer.writeheader()
            for detail in grade_details:
                writer.writerow(grading_detail_csv_row(detail))
    ...
    report.add_keyword_grading_report(summary_lines, csv_path=csv_path)
    ...
    if csv_path:
        write_stats["分级明细CSV"] = str(csv_path.name)
